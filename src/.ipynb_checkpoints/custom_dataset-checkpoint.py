...
from src.image_augmentation import *
...
class CropData(Dataset):
    ...
    def __init__(self, src_dir, usage, dataset_name, csv_path, apply_normalization=False, 
        ...
        img_fnames = [Path(dirpath) / f
                      ...
                      for f in filenames 
                      if f.endswith(".tif") and ("_false_color" in f) and ('_'.join(Path(f).stem.split('_')[:4]) in flag_ids)]
        img_fnames.sort()
        ...
        lbl_fnames = [Path(dirpath) / f 
                      for (dirpath, dirnames, filenames) in os.walk(Path(src_dir) / self.dataset_name) 
                      for f in filenames 
                      if f.endswith(".tif") and ("mask" in f) and ('_'.join(Path(f).stem.split('_')[:4]).split(".")[0] in flag_ids)]
        lbl_fnames.sort()
        # print(lbl_fnames)
